...
class unipico_application_class:
    ...
    def __init__(self):
        ...
        self.button_sense = self.BUTTON_SENSE_MAX
        ...
        self.menu = [
                ...
                [('MIN:LOAD',    '', None),              ('SET:',  '{:03d}', self.get_midi_set)]
            ]
    ...
    def set_default(self):
        midi_in_set = midi_in_player_obj.read_midi_in_settings(0)
        if not midi_in_set is None:
            print('LOAD MIDI-IN DEFAULT:', midi_in_set)
    ...
    def get_midi_set(self, delta=0):
        if delta != 0:
            ...
            if not midi_in_set is None:
                ...
                midi_in_player_obj.set_midi_in_setting(midi_in_set)
                midi_in_player_obj.send_all_midi_in_settings()
                self.midi_in_save_file = set_num
    ...
    def get_rev_prog(self, delta=0):
        if delta != 0:
            ...
            value = effector[0]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'reverb', 0, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_rev_levl(self, delta=0):
        if delta != 0:
            ...
            value = effector[1]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'reverb', 1, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_rev_fdbk(self, delta=0):
        if delta != 0:
            ...
            value = effector[2]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'reverb', 2, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_chr_prog(self, delta=0):
        if delta != 0:
            effector = midi_in_player_obj.get_midi_in_setting(self.midi_channel, 'chorus')
            value = effector[0]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'chorus', 0, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_chr_levl(self, delta=0):
        if delta != 0:
            ...
            value = effector[1]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'chorus', 1, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_chr_fdbk(self, delta=0):
        if delta != 0:
            ...
            value = effector[2]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'chorus', 2, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_chr_dely(self, delta=0):
        if delta != 0:
            ...
            value = effector[3]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'chorus', 3, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_vib_rate(self, delta=0):
        if delta != 0:
            effector = midi_in_player_obj.get_midi_in_setting(self.midi_channel, 'vibrate')
            value = effector[0]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'vibrate', 0, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_vib_dept(self, delta=0):
        if delta != 0:
            ...
            value = effector[1]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'vibrate', 1, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def get_vib_dely(self, delta=0):
        if delta != 0:
            ...
            value = effector[2]
            value = (value + delta) % 128
            midi_in_player_obj.set_midi_in_setting4(self.midi_channel, 'vibrate', 2, value)
            midi_in_player_obj.send_midi_in_settings(self.midi_channel)
    ...
    def device_joystick_controller(self, joy_x, joy_y, joy_b):
        ...
        # Joystick button works as a tact switch to avoid chattering
        if joy_b:
            if self.button_sense > 0:
                self.button_sense = self.button_sense - 1
                if self.button_sense > 0:
                    joy_b = 0
            else:
                joy_b = 0
        ...
        if joy_b == True and self.joystick_b == False:
            # MIDI-IN Player joystick control mode on/off
            if self.menu_selected == self.MENU_MIN_PLAY_CTRL:
                self.midi_in_player_controller = not self.midi_in_player_controller
                self.show_menu()

            # Sequencer Player control
            ...
            # MIDI-IN set file save action
            elif self.menu_selected == self.MENU_MIN_SAVE:
                print('SAVE MIDI-IN SET:', self.midi_in_save_file)
                midi_in_player_obj.write_midi_in_settings(self.midi_in_save_file)
                print('SAVED')
